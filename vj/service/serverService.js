...
const ServerService = {
    ...
    getManifest() {
        return fetch(`${MOON_BASE}rad-moon-manifest`).then(
            response => {
    ...
    getNextSidx(id) {
        let p = Q.resolve(
            $.get(SERVER_BASE + "getNextVideoSidx", {
                id: id,
            })
        );
    ...
    getNextYoutubeFromPlaylist(obj, options) {
        let p = Q.resolve(
            $.get(SERVER_BASE + "getNextVideoFromPlaylist", {
                ...obj,
                ...options,
            })
        );
    ...
    getSidx(id, options) {
        ...
        console.log(id);
        return youtubeSocket
            .getManifest(type, id, null, {
    ...
    channelUploadsFromComments(results, userProfile, existingIds) {
        ...
        let mapped = Q.map(
            channelIds,
            chId => {
                let chUpload = this.channelUploads(chId).then(
                    data => {
                        if (data.length) {
                            _.each(data, item => {
                                //is not a likes video
                                let vId = Utils.extractVideoIdFromUpload(
                                    item.img
                                );
                                let _views = Utils.extractViewsFromScrape(
                                    item.content
                                );
                                if (existingIds.indexOf(vId) < 0) {
                                    if (
                                        ...
                                        userProfile.uploads.push({
                                            videoId: vId,
                                            views: _views,
                                        });
                                    } else {
                                        userProfile.likes.push({
                                            videoId: vId,
                                            views: _views,
                                        });
                                    }
                            ...
                            if (
                                channelIds.indexOf(chId) > 5 &&
                                !userProfile.uploads.length
                            ) {
                                console.log(
                                    "Couldn't find any uploads, using likes"
                                );
                                return chUpload.cancel();
                            } else if (userProfile.uploads.length) {
                                console.log(
                                    `Found uploads of ${chId}`
                                );
                                return chUpload.cancel();
                            }
                            return data;
