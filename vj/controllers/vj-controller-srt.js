...
class SrtController extends ControllerBase {
    ...
    initLoad(init) {
        return Q.all(this._options.audioPlaylist)
            .map(url => this.getJSONManifest(url))
            .then(audioPlaylists => {
                this.manifests = _.flatten(audioPlaylists);
                // this._subtitles.forEach(sub => (sub.videoId = _.sample(this.manifests).videoId));
                // this.manifests.forEach(m => (m.subs = _.sample(manifest).subs));
                if (init) {
                    this._init();
                }
    ...
    _init() {
        ...
        var _self = this;
        this._ii = setInterval(function() {
            if (_self._isReady) {
                clearInterval(_self._ii);
            }
        }, 100);
    }

    _chooseSubs() {
        return this._phrase.chooseSubs(this._subtitles, this._options.subsPerPhrase, false);
    }
    ...
    getVo(subs) {
        ...
        //     _o.audioonly = true
        // }
        return this._findSidxsFromSubs(subs).then(sidxs => {
            //console.log();
            if (_.compact(sidxs).length) {
    ...
    _getSidx(id, options = this._queryOptions) {
        ...
        if (this.manifests) {
            ...
            while (!sidx) {
                ...
                sidx = manifest.sidx;
                sidx.url = `${ASSETS}videos/${manifest.videoId}_140`;
                sidx.codecs = manifest.codecs;
                sidx.indexRange = manifest.indexRange;
                sidx.videoId = id;
            }
            return sidx;
        }
    ...
    _findNewSentence(randomSentence = false) {
        return new Q((resolve, reject) => {
            ...
            if (!this._sidxs[_videoId]) {
                this._newSidx(_videoId, this._options).then(sidx => {
                    return _findSub();
                });
            } else {
                _findSub();
    ...
    _nextSub(randomFactor = RANDOM_FACTOR) {
        return new Q((resolve, reject) => {
            ...
            let _sub,
                _videoSubs,
                _videoId,
                _videoIndex = this.activeVideoId;
